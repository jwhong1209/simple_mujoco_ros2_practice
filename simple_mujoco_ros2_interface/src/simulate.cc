...
void ShowOverlayText(mj::Simulate * sim, mjrRect viewport, int font, int gridpos, std::string text1,
                     std::string text2)
{
                    ...
                    char (&content)[mj::Simulate::kMaxFilenameLength])
{
  ...
  mju::sprintf_arr(
    ...
    solerr, solver_niter, fps, 100 * d->maxuse_arena / (double)(d->narena),
  ...
  for (int i = 0; i < sim->jnt_type_.size() && itemcnt < mjMAXUIITEM; i++)
  {
    if ((sim->jnt_type_[i] == mjJNT_HINGE || sim->jnt_type_[i] == mjJNT_SLIDE))
    {
      ...
      if (sim->jnt_range_[i].has_value())
        mju::sprintf_arr(defSlider[0].other, "%.4g %.4g", sim->jnt_range_[i]->first,
                         sim->jnt_range_[i]->second);
...
void CopyCamera(mj::Simulate * sim)
{
  ...
  mju::sprintf_arr(
    clipboard, "<camera pos=\"%.3f %.3f %.3f\" xyaxes=\"%.3f %.3f %.3f %.3f %.3f %.3f\"/>\n",
...
void UiEvent(mjuiState * state)
{
  ...
  if (state->type == mjEVENT_SCROLL && state->mouserect == 3)
  {
    ...
    {
      mjv_moveCameraFromState(&sim->scnstate_, mjMOUSE_ZOOM, 0, -zoom_increment * state->sy,
                              &sim->scn, &sim->cam);
    }
    return;
  }
  ...
  if (state->type == mjEVENT_MOVE && state->dragrect == 3 && (sim->m_ || sim->is_passive_))
  {
    ...
    if (sim->pert.active)
    {
      if (!sim->is_passive_)
      {
        mjv_movePerturb(sim->m_, sim->d_, action, state->dx / r.height, -state->dy / r.height,
                        &sim->scn, &sim->pert);
      }
      else
      {
    ...
    else
    {
      if (!sim->is_passive_)
      {
        mjv_moveCamera(sim->m_, action, state->dx / r.height, -state->dy / r.height, &sim->scn,
                       &sim->cam);
      }
      else
      {
...
Simulate::Simulate(std::unique_ptr<PlatformUIAdapter> platform_ui, mjvCamera * cam, mjvOption * opt,
...
#undef X
  ...
  if (pending_.select)
  {
    ...
    int selbody = mjv_select(m_, d_, &this->opt, static_cast<mjtNum>(r.width) / r.height,
                             (pending_.select_state.x - r.left) / r.width,
                             (pending_.select_state.y - r.bottom) / r.height, &this->scn, selpnt,
  ...
  else
  {
    ...
    if (user_scn)
    {
      ...
      if (ngeom > 0)
      {
        std::memcpy(scnstate_.scratch.geoms + scnstate_.scratch.ngeom, user_scn->geoms,
...
void Simulate::LoadOnRenderThread()
{
  ...
  for (int i = 0; i < this->m_->nu; ++i)
  {
    ...
    {
      actuator_ctrlrange_.push_back(std::make_pair(this->m_->actuator_ctrlrange[2 * i],
...
void Simulate::Render()
{
  ...
  if (desiredRealtime != 100.0 || misaligned)
  {
    ...
    if (misaligned)
    {
      std::snprintf(rtlabel + labelsize, sizeof(rtlabel) - labelsize, " (%-4.1f%%)",
                    actualRealtime);
    }
  }
